...
ChatRemote.prototype.add2 = function(uid, sid, rid, flag, cb) {
	...
	this.channelService.pushMessageByUids('onAddCommonUser',{rid:rid,num:numCnt},uidArray,function(err){
        if(err){
            console.log(err);
            return;
        }
	...
	this.channelService.pushMessageByUids('onCommonUserAddToGame',{rid:rid,uid:uid},uidArrayRid,function(err){
        if(err){
            console.log(err);
            return;
        }
